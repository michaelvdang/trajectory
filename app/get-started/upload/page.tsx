      ...
      <div className="flex flex-col items-center justify-center py-8 bg-white">
        <div className="max-w-7xl w-full flex flex-col items-center">
          <div className="flex justify-center items-center pt-16">
            <h1 className="text-4xl font-bold mb-6 text-black">Upload Your Resume</h1>
          </div>
          <div className="relative flex flex-col justify-center items-center gap-6 bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg p-12 md:p-24 w-11/12 md:w-3/4 border-violet-300/15 shadow-[0_0_80px_inset] shadow-violet-300/15 mx-auto">
            <FileUploader
              value={files}
              onValueChange={setFiles}
              dropzoneOptions={dropZoneConfig}
            >
              <FileInput className="outline-dashed outline-2 outline-purple-300">
                <div className="flex items-center justify-center flex-col pt-3 pb-4 w-full text-purple-700">
                  <FileSvgDraw />
                </div>
              </FileInput>
              <FileUploaderContent>
                {files &&
                  files.length > 0 &&
                  files.map((file, i) => (
                    <FileUploaderItem key={i} index={i} className="flex items-center text-purple-700">
            ...
            {files && files.length > 0 && (
              <Button
                className="mt-8 px-6 py-3 w-1/4 bg-purple-800 text-white rounded-lg hover:bg-purple-900 transition-colors"
          ...
          <div className="max-w-xl md:max-w-3xl py-12 px-12 md:px-24 w-full flex justify-between gap-6">
            <Link href="/" className="text-purple-800 hover:text-purple-900 transition-colors">
              Prev
            </Link>
            <Link href="/get-started/target-job" className="text-purple-800 hover:text-purple-900 transition-colors">
              Skip
            </Link>
            {uploadStatus === 'successful' ? (
              ...
              <Button disabled variant="default" className="text-gray-400 cursor-not-allowed">
                Next
              </Button>
            )}
          </div>
